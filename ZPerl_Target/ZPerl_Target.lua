...
function XPerl_Target_Set_Bits(self)
	...
	if (self == XPerl_Focus) then
		XPerl_Target_Set_FocusCPFrame(self)
	end
...
function XPerl_Focus_ComboFrame_Update()
	local comboPoints = GetComboPoints("player", "focus")
	if comboPoints > 0 then
		if not FocusComboFrame:IsShown() then
			FocusComboFrame:Show()
			UIFrameFadeIn(FocusComboFrame, COMBOFRAME_FADE_IN)
		end

		-- local comboIndex = 1
		for i = 1, 5 do
			local fadeInfo = {}
			-- local comboPoint = FocusComboFrame.ComboPoints[comboIndex]
			local comboPoint = _G["FComboPoint"..i]
			if i < 6 then
				comboPoint:Show()
			else
				if comboPoints >= i then
			...
			if i <= comboPoints then
				if i > (FocusComboFrame.lastPoints or 0) then
					-- Fade in the highlight and set a function that triggers when it is done fading
					fadeInfo.mode = "IN"
					fadeInfo.timeToFade = COMBOFRAME_HIGHLIGHT_FADE_IN
					fadeInfo.finishedFunc = ComboPointShineFadeIn
	...
	FocusComboFrame.lastPoints = comboPoints
end
...
function XPerl_Target_Set_FocusCPFrame(self)
	if fconf.combo.blizzard then
		...
		if fconf.combo.pos == "top" then
			FocusComboFrame:SetPoint("TOP", self.portraitFrame, "TOP", 98, 4)
			FComboPoint1:SetPoint("TOPLEFT", 0, 0)
			FComboPoint2:SetPoint("LEFT", FComboPoint1, "RIGHT", 0, 1)
			FComboPoint3:SetPoint("LEFT", FComboPoint2, "RIGHT", 0, 1)
			FComboPoint4:SetPoint("LEFT", FComboPoint3, "RIGHT", 0, -1)
			FComboPoint5:SetPoint("LEFT", FComboPoint4, "RIGHT", 0, -1)
		elseif fconf.combo.pos == "bottom" then
			FocusComboFrame:SetPoint("BOTTOM", self.portraitFrame, "BOTTOM", 98, -4)
			FComboPoint1:SetPoint("BOTTOMLEFT", 0, 0)
			FComboPoint2:SetPoint("LEFT", FComboPoint1, "RIGHT", 0, -1)
			FComboPoint3:SetPoint("LEFT", FComboPoint2, "RIGHT", 0, -1)
			FComboPoint4:SetPoint("LEFT", FComboPoint3, "RIGHT", 0, 1)
			FComboPoint5:SetPoint("LEFT", FComboPoint4, "RIGHT", 0, 1)
		elseif fconf.combo.pos == "left" then
			FocusComboFrame:SetPoint("BOTTOMLEFT", self.portraitFrame, "BOTTOMLEFT", -1, 0)
			FComboPoint1:SetPoint("BOTTOMLEFT", 0, 0)
			FComboPoint2:SetPoint("BOTTOM", FComboPoint1, "TOP", -1, 0)
			FComboPoint3:SetPoint("BOTTOM", FComboPoint2, "TOP", -1, 0)
			FComboPoint4:SetPoint("BOTTOM", FComboPoint3, "TOP", 1, 0)
			FComboPoint5:SetPoint("BOTTOM", FComboPoint4, "TOP", 1, 0)
		elseif fconf.combo.pos == "right" then
			FocusComboFrame:SetPoint("BOTTOMRIGHT", self.portraitFrame, "BOTTOMRIGHT", 2, 0)
			FComboPoint1:SetPoint("BOTTOMRIGHT", 0, 0)
			FComboPoint2:SetPoint("BOTTOM", FComboPoint1, "TOP", 1, 0)
			FComboPoint3:SetPoint("BOTTOM", FComboPoint2, "TOP", 1, 0)
			FComboPoint4:SetPoint("BOTTOM", FComboPoint3, "TOP", -1, 0)
			FComboPoint5:SetPoint("BOTTOM", FComboPoint4, "TOP", -1, 0)
		...
		FocusComboFrame:RegisterEvent("PLAYER_FOCUS_CHANGED")

		FocusComboFrame:SetScript("OnEvent", XPerl_Target_ComboFrame_OnEvent)
		FocusComboFrame:SetParent(XPerl_Focus)

		XPerl_Focus_ComboFrame_Update()
	else
		FocusComboFrame:Hide()
		FocusComboFrame:UnregisterEvent("PLAYER_FOCUS_CHANGED")
	end
end
